...
#   2. 以 dd 填充 /overlay，直到剩餘空間 < 5 % 或剩餘空間 < 1GB，立即呼叫
...
FILL_THRESHOLD=5
FILL_STEP_MB=50
LOWER_MB_LIMIT=1024  # 最少保留 1GB 空間
OVERLAY_DIR="/overlay"
...
###############################################################################
# 2. 模擬磁碟空間不足 (<5%) 或剩餘空間 < 1GB，驗證自動清理
###############################################################################
echo "\n🚨 低空間測試：填充 $OVERLAY_DIR 直到剩餘 < ${FILL_THRESHOLD}％ 或可用 < ${LOWER_MB_LIMIT}MB"
while : ; do
  USED=$(df "$OVERLAY_DIR" | awk 'NR==2 {gsub("%","",$(NF-1)); print $(NF-1)}')
  FREE_MB=$(df "$OVERLAY_DIR" | awk 'NR==2 {print $(NF-2)}')
  [ "$USED" -ge $((100 - FILL_THRESHOLD)) ] && break
  [ "$FREE_MB" -lt "$LOWER_MB_LIMIT" ] && {
    echo "🛑 可用空間已低於 ${LOWER_MB_LIMIT}MB，停止填充！"
    break
  }
  dd if=/dev/zero of="$FILL_FILE" bs=1M count=$FILL_STEP_MB oflag=append conv=notrunc 2>/dev/null
  printf "  ‣ 已填充，剩餘空間：${FREE_MB}MB\n"
done
...
echo "    • 空間不足或可用空間太低時 system_health_check.sh 有自動清除備份"
